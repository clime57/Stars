...
public class ExcelTools : Editor
{
    ...
    private const string STR_NOT_FILE = "Data文件夹下没有任何EXCEL文件";
    ...
    private static void CreateCsFile(string fileName, DataTable dataTable)
    {
       ...
       classStrValue.Append("public class " + fileName + " :IConfig{" + "\r\n");
        ...
        {
            string variableValue = dataTable.Rows[1][i].ToString();
            string notesValue = "";//dataTable.Rows[1][i].ToString();//注释文本

            if (dataTable.Rows[0][i].ToString() == "2") //== ""通用 == 1仅前端用  == 2仅后端用
                continue;

            string ID = (string)dataTable.Rows[2][i];
            if (i == 0 && ID != "id")
            {
                string tempID = ID;
                ID = "id";
    ...
    {    
        ...
        for (int w = 0; w < dataTable.Rows.Count; w++)
        {
            ...
            for (int s = 0; s < dataTable.Columns.Count; s++)
            {
                ...
                if (needValue != "1")
                    iData.SetAttribute((dataTable.Rows[2][s]).ToString(), (dataTable.Rows[w][s]).ToString());
            }
    ...
    private static void SerializeHandler(Assembly assembly, DataTable dataTable, string fileName)
    {
        ...
        object[] dataArr = new object[rows - 3];
        object obj = null;
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            for (int j = 0; j < columns; j++)
            {
                ...
                if (firstValue == "2") //== ""通用 == 1仅前端用  == 2仅后端用
                    continue;

                if (i < 3)// || dataTable.Rows[i][0].ToString() == string.Empty
                {
                    obj = null;
                    continue;
                }
                ...
                if(j == 0)
                {
                    ...
                    {
                        TryClearProgressBar();
                        EditorUtility.DisplayDialog(STR_WARNING, "excel中的id不能为空,行：" + (i  +1)+ " 路径：" + NOW_PATH, STR_CONFIRM);
                ...
                string variableValueType = dataTable.Rows[1][j].ToString();//字段类型行
                string variableValue = dataTable.Rows[2][j].ToString();//字段行
