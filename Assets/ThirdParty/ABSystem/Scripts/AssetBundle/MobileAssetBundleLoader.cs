...
namespace Stars
{
    ...
    public class MobileAssetBundleLoader : AssetBundleLoader
    {
        ...
        protected string _assetBundleSourceFileCS;
        ...
        void LoadDependsSync()
        {
            ...
            for (int i = 0; i < depLoaders.Length; i++)
            {
                ...
                if (!depLoader.isComplete)
                {
                    depLoader.LoadBundleSync();
                }
            }
        }

        override public AssetBundleInfo LoadBundleSync()
        {
            _assetBundleCachedFile = string.Format("{0}/{1}", bundleManager.pathResolver.BundleCacheDir, bundleName);
            _assetBundleSourceFileCS = bundleManager.pathResolver.GetBundleSourceFile(bundleName,false);
            LoadDependsSync();
            if (File.Exists(_assetBundleCachedFile))
                return LoadFromCachedFileSync();
            else
                return LoadFromPackageSync();
        }
        ...
        protected AssetBundleInfo LoadFromPackageSync()
        {
            if (state != LoadState.State_Error)
            {
                _bundle = AssetBundle.LoadFromFile(_assetBundleSourceFileCS);
        ...
        protected AssetBundleInfo LoadFromCachedFileSync()
        {
            if (state != LoadState.State_Error)
            {
                //兼容低版本API
